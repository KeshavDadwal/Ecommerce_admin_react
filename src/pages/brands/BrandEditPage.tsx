...
export const BrandEditPage = () => {
    ...
    useEffect(() => {
        const brand = (brandData as any)?.brand;
        if (!brand) return;
        setForm({
            name: brand.name ?? '',
        ...
        if (brand.imageUrl) setImagePreview(brand.imageUrl);
    }, [brandData]);
        ...
        try {
            ...
            if (imageFile) {
                const operations = JSON.stringify({
                    query: `mutation UpdateBrand($id: ID!, $input: UpdateBrandInput!) {
                        updateBrand(id: $id, input: $input) { id name }
                    }`,
                ...
                await updateBrand({ variables: { id, input: inputPayload } });
            }
    ...
    const existingImageUrl = (brandData as any)?.brand?.imageUrl;
